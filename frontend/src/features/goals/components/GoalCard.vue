...
import { Text, Box, InputField, Button } from "@/shared/components/ui";
import { CheckOutline, XMark } from "@/shared/components/icons";
import { useUpdateGoal, useDeleteGoal } from "@/features/goals/queries";
...
async function handleDeleteGoal(e: MouseEvent) {
  ...
  deleteGoalMutation(props.goal.id, {
    onSuccess: () => {
      setIsEditing(false);
      setIsDeleting(false);
    },
